...
interface ILiqualityHTLC {
    ...
    /// @notice Emitted when the provided expiration is smaller than the current block timestamp
    error LiqualityHTLC__InvalidExpiration();

    /// @notice Emitted when the amount is 0
    error LiqualityHTLC__InvalidSwapAmount();

    /// @notice Emitted when initiating TRX HTLC and the amount does not match the msg.value
    ...
    /// @notice Emitted when the generated swap does not exist
